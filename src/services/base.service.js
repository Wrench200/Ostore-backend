const { bcrypt, jwt, SECRET_TOKEN, REFRESH_TOKEN } = require('../../configs/app.config');
...
class BaseService {
    ...
    token(data) {
        ...
        return jwt.sign(data, SECRET_TOKEN, { expiresIn: "86400000" });
    }
    refreshToken(data) {
        return jwt.sign(data, REFRESH_TOKEN, { expiresIn: "604800000 " });
    }

    tokenVerify(token, refresh = false) {
        console.log(token);
        try {
            if (refresh == true) {
